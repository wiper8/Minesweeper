homologous_next_i <- function(grid, next_i, mines_left, solved_around, in_cluster, dims = dim(grid)) {
  ...
  keep <- apply(homologous_candidates, 1, function(pos_x) !grid[pos_x[1], pos_x[2]] %in% known && in_cluster[pos_x[1], pos_x[2]])
  ...
  if (nrow(influence) == 0) return(next_i)
  ...
  position_to_i_mat(homologous_candidates, dims)
}
